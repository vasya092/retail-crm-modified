...
class WC_Retailcrm_Order_Item extends WC_Retailcrm_Abstracts_Data
{
    ...
    public function build($item)
    {
        ...
        if (isset($item->get_product()->get_data()['sale_price']) && $price == $item->get_product()->get_data()['sale_price']) {
            $data['properties'][] = [
                'code' => 'sale_discount',
                'name' => 'Акционная скидка',
                'value' => $item->get_product()->get_data()['regular_price'] - $item->get_product()->get_data()['sale_price']
            ];
        }
